...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
    ...
    list_for_each_entry(entry, list, hash_list) 
    {
        ...
        {
            return entry; // 找到匹配的TCP套接字，返回
        }
    }

    return NULL; // 未找到匹配的TCP套接字，返回NULL
}
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
    ...
    list_for_each_entry(entry, list, hash_list) 
    {
        ...
        {
            return entry; // 找到匹配的监听套接字，返回
        }
    }

    return NULL; // 未找到匹配的监听套接字，返回NULL
}
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
    ...
    if (tcp_sock_set_sport(tsk, sport) == -1) {
        return -1; // 如果无法设置源端口，返回错误
    }

    rt_entry_t *entry = longest_prefix_match(ntohl(skaddr->ip));
    tsk->sk_sip = entry->iface->ip;          // 设置源IP地址
    tsk->sk_dport = ntohs(skaddr->port);     // 设置目的端口
    tsk->sk_dip = ntohl(skaddr->ip);         // 设置目的IP地址

    // 2. 将TCP套接字插入绑定表
    tcp_bind_hash(tsk);

    // 3. 发送SYN包，切换到TCP_SYN_SENT状态，并在wait_connect上休眠，等待SYN-ACK
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
    ...
    // 从接收队列中取出第一个连接的TCP套接字
    struct tcp_sock *pop = tcp_sock_accept_dequeue(tsk);

    // 设置状态为TCP_ESTABLISHED并将套接字插入哈希表
...
void tcp_sock_close(struct tcp_sock *tsk)
{
    switch (tsk->state) {
        case TCP_LISTEN:
            // 从哈希表中移除，并将状态设置为TCP_CLOSED
            tcp_unhash(tsk);
            tcp_set_state(tsk, TCP_CLOSED);
            break;

        case TCP_ESTABLISHED:
            // 发送FIN + ACK包，并将状态设置为TCP_FIN_WAIT_1
            tcp_send_control_packet(tsk, TCP_FIN | TCP_ACK);
            tcp_set_state(tsk, TCP_FIN_WAIT_1);
            break;

        case TCP_CLOSE_WAIT:
            // 发送FIN + ACK包，并将状态设置为TCP_LAST_ACK
            tcp_send_control_packet(tsk, TCP_FIN | TCP_ACK);
